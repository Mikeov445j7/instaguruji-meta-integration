...
export class BlogContentPage implements OnInit {
  ...
  onLoadData(event?: any) {
    let blogId: string = getParamValueQueryString('blogId');
    this._common.showLoader();
    forkJoin([
      this._homeService.BlogById(blogId),
      this._homeService.BlogContentById(blogId)
  ...
  async showHideBanner(flag: boolean = true): Promise<void> {
    if (ENV.platform == PLATFORM_DEVICE.ANDROID) {
      if (flag) {
        await this.adMobInstance.showBanner(BANNER_ADD_OPTIONS('blog_content'));
